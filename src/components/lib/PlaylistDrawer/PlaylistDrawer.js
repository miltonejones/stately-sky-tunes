...
import { styled, Drawer, Typography,  Box } from '@mui/material';
  ...
  <>
   <Drawer anchor="left" open={open} onClose={() => send('CLOSE')}>
    ...
    <Box sx={{width: 400, m: 2}}>
      {playlists.records.map(playlist => <Flex onClick={() => handleEdit(playlist.listKey || createKey(playlist.Title))} sx={{mb: 1, width: 380}} spacing={1}>
          <Avatar src={playlist.image} />
          <Typography variant="body2">{playlist.Title}</Typography>
          <Spacer />
          <i className={`fa-${playlist.related.indexOf(track.FileKey) > -1 ?"solid":"regular"} fa-heart`}/> 
      </Flex>)} 
    </Box>
  {JSON.stringify(state.value)}
     {/* <pre>
      {JSON.stringify(playlists, 0, 2)}
     </pre> */}
   </Drawer>
  </>
 );
