...
export const playlistMachine = createMachine({
  ...
  context: { open: false, playlists: [], playlist_db: [], track: null, listname: "" },
  ...
  const [state, send] = useMachine(playlistMachine, {
    services: { 
      editList: async(context) => {
        ...
        const playlist = playlists.records.find(f => createKey(f.Title) === listname);
        if (playlist) {
          const  { related } = playlist;
          const updated = {
            ...playlist,
            related: related.indexOf(track.FileKey) > -1 
